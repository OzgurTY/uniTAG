...
import { NotificationService } from '@/src/services/notificationService'; // EKLENDİ
...
import { ActivityIndicator, FlatList, RefreshControl, SafeAreaView, StyleSheet, Switch, Text, TouchableOpacity, View } from 'react-native';
...
export default function HomeScreen() {
  ...
  const [unreadCount, setUnreadCount] = useState(0); // Bildirim Sayısı

  // 1. Bildirimleri Dinle
  useEffect(() => {
    if (user) {
  ...
  const toggleDriverMode = () => {
    ...
    if (profile?.driverDetails && profile.driverDetails.plateNumber) {
      ...
      // Basit alert, driver/register'a yönlendirebilirsin
      router.push('/driver/register'); 
    }
  };

  // HEADER (Zil İkonu Burada)
  const ListHeader = () => (
    <View style={styles.header}>
      ...
      <View style={{ flexDirection: 'row', gap: 16, alignItems: 'center' }}>
        ...
        <TouchableOpacity onPress={() => router.push('/notifications')}>
          <View>
            <IconSymbol name="bell.fill" size={24} color={Colors.textDark} />
            {unreadCount > 0 && (
              <View style={styles.badge}>
                 {/* İçi boş kırmızı nokta */}
              </View>
            )}
          </View>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  // ... MEVCUT STİLLER AYNEN KALSIN ...
  container: { flex: 1, backgroundColor: Colors.background },
  header: { paddingHorizontal: 24, paddingVertical: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', borderBottomWidth: 1, borderBottomColor: Colors.border, marginBottom: 16 },
  ...
  sectionTitle: { fontSize: 18, fontWeight: '700', color: Colors.textDark, marginHorizontal: 24, marginBottom: 12 },
  emptyState: { padding: 40, alignItems: 'center' },
  emptyText: { color: Colors.textMedium, textAlign: 'center' },
